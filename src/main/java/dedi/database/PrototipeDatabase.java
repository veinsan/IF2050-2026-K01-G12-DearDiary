...
public class PrototipeDatabase {
    ...
    private static final String COLUMNS =
        "id_prototipe, id_ide, versi, status, deskripsi_perubahan, tanggal_perubahan";

    private static final String INSERT_SQL =
        "INSERT INTO prototipe (id_ide, versi, status, deskripsi_perubahan, tanggal_perubahan) " +
        "VALUES (?, ?, ?, ?, ?)";

    private static final String UPDATE_SQL =
        "UPDATE prototipe SET versi = ?, status = ?, deskripsi_perubahan = ?, tanggal_perubahan = ? " +
        "WHERE id_prototipe = ?";
    ...
    public void insertPrototipe(Prototipe p) {
        try {
            ...
            try (PreparedStatement ps = conn.prepareStatement(INSERT_SQL, Statement.RETURN_GENERATED_KEYS)) {
                ...
                if (p.getTanggalPerubahan() != null){
                    ps.setDate(5, Date.valueOf(p.getTanggalPerubahan()));
                } else {
    ...
    public boolean updatePrototipe(Prototipe p) {
        try {
            ...
            try (PreparedStatement ps = conn.prepareStatement(UPDATE_SQL)) {
                ...
                if (p.getTanggalPerubahan() != null) {
                    ps.setDate(4, Date.valueOf(p.getTanggalPerubahan()));
                } else {
                    ps.setNull(4, java.sql.Types.DATE);
                }
                ps.setInt(5, p.getIdPrototipe());
    ...
    private Prototipe mapRow(ResultSet rs) throws SQLException {
        Date sqlDate = rs.getDate("tanggal_perubahan");
        LocalDate tanggalPerubahan = (sqlDate != null) ? sqlDate.toLocalDate() : null;
